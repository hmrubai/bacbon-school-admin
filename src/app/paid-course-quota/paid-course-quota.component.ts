...
@Component({
    selector: 'app-paid-course-quota',
    templateUrl: './paid-course-quota.component.html',
    styleUrls: ['./paid-course-quota.component.scss'],
    encapsulation: ViewEncapsulation.None
})
export class PaidCourseQuotaComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.quotaForm = this.formBuilder.group({
            ...
            old_access_count: [null, [Validators.required, Validators.maxLength(250)]],
            new_access_count: [null, [Validators.required, Validators.maxLength(250)]],
        });
    ...
    filterResultList(course_meterial){
        if(course_meterial){
            ...
            this._service.get('paid-course/quota-list/' + course_meterial.id).subscribe(res => {
    ...
    filterResultListWithMobile(){
        if(this.meterial_form && this.mobile_no){
            let params = {
            ...
            this.loadingIndicator = true;
            this.is_meterial_selected = true;
            this._service.get('paid-course/quota-list/' + this.meterial_form, params).subscribe(res => {
                this.paidCourseResultList = res.result;
                this.is_meterial_selected = true;
                setTimeout(() => {
                    this.loadingIndicator = false;
            ...
            if(!this.meterial_form){
                this.toastr.warning("Please, select test!", 'Warning!', { closeButton: true, disableTimeOut: false });
            }
            this.is_meterial_selected = false;
            this.meterial_form = null;
        }
    ...
    updateQuota(quota, template: TemplateRef<any>){
        console.log(quota)

        this.quotaForm.controls['id'].setValue(quota.id);
        this.quotaForm.controls['old_access_count'].setValue(quota.access_count);
        this.quotaForm.controls['new_access_count'].setValue(quota.access_count);
    ...
    onQoutaFormSubmit() {
        ...
        if(this.quotaForm.value.old_access_count > this.quotaForm.value.new_access_count){
            this.toastr.warning('Please, check details.', 'Attention!', { closeButton: true, disableTimeOut: false });
        }

        this.blockUI.start('Updating data. Please wait...');
        this._service.post('paid-course/update-quota', this.quotaForm.value).subscribe(
            data => {
                this.blockUI.stop();
    ...
    downloadSubjectWiseResult(){
        this.blockUI.start('Generating report. Please wait...');
        this._service.downloadFile('web/paid-course-subject-wise-all-result/' + this.meterial_form).subscribe(res => {
            this.blockUI.stop();

            const url = window.URL.createObjectURL(res);
            var link = document.createElement('a');
            link.href = url;
            link.download = "Paid Course Test Subject Wise Result";
            link.click();
        },
        err => {
            this.toastr.warning(err.messages || err, 'Warning!');
