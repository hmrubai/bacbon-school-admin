...
@Component({
  selector: 'app-school-wise-learner',
  templateUrl: './school-wise-learner.component.html',
  styleUrls: ['./school-wise-learner.component.scss'],
  encapsulation: ViewEncapsulation.None
})

export class SchoolWiseLearnerComponent implements OnInit {
  ...
  ngOnInit() {
    this.getSchoolWiseClasses();
  }

  

  getSchoolWiseClasses() {
  ...
  loadClassBySchool() {
    this.schools.forEach(school => {
      ...
      school.Classes.forEach(element => {
        element.Learners = null;
        element.Total = null;
        this.GetLearnerBySchoolClassId(school,element);
      });
    });
  }

  GetLearnerBySchoolClassId(school: any,item: any) {
    const obj = {
      size: this.page.size,
   ...
   this._service.get('Learner/GetLearnerBySchoolClassId',obj ).subscribe(res => {
      this.blockUI.stop();
      if (res.Status === 2) {
         this.toastr.warning(res.Message, 'Warning!', { timeOut: 2000 });
         return;
      ...
      item.Learners = res.Data.List;
      item.Total = res.Data.TotalRows;
