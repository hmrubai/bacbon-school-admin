...
export class CouponComponent implements OnInit {
    ...
    getCourseList() {
        this.blockUI.start('Getting data. Please wait...');
        this._service.get('paid-course/get-all-active-courses').subscribe(res => {
            this.paidCourseList = res.result;
            this.blockUI.stop();
        }, err => {
            this.toastr.warning(err.messaage || err, 'Warning!', { closeButton: true, disableTimeOut: false });
            this.blockUI.stop();
    ...
    downloadAsExcel(){
        ...
        this._service.downloadFile('paid-course/download-coupon-list').subscribe(res => {
            this.blockUI.stop();

            const url = window.URL.createObjectURL(res);
            var link = document.createElement('a');
            link.href = url;
            link.download = "Paid Course - Coupon List";
            link.click();
        },
        err => {
